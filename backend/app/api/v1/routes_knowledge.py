...
async def upload_document(file: UploadFile = File(...), user=Depends(require_roles("owner", "admin")), db: Session = Depends(get_db)):
...
def delete_document(document_id: int, user=Depends(require_roles("owner", "admin")), db: Session = Depends(get_db)):
